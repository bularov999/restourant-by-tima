...
export class AuthService {
  ...
  async authorization(authDto: AuthDto): Promise<ResponseMessageDto> {
    ...
    if (candidate) {
      const candidateCreatedDateTimeInMiliseconds =
        candidate.createdDateTime.getTime();
      if (candidateCreatedDateTimeInMiliseconds + SPAM_TIME > Date.now()) {
        throw ApiError.badRequest(
          'you can not sign in before 5 minutes have not passed',
        );
      }
      try {
        const newAuth: AuthEntity = await this.createAuthEntity(
          candidate.phone,
          candidate,
        );
        return {
  ...
  async verifyMessage(
    ...
    if (!authData)
      throw ApiError.badRequest('there is no user with that id and code');
    ...
    const user = await this.userService.findOne({
      where: { id: verifyMessageDto.userId },
    });
  ...
  async loginAsAdmin(adminAuthDto: AdminAuthDto) {
    const admin = await this.userService.findOne({
      where: { email: adminAuthDto.email },
    });
    if (!admin) throw ApiError.notFound('admin not found');
    const comparedPassword = comparePasswordByBcrypt(
      adminAuthDto.password,
      admin.password,
    );
    if (!comparedPassword)
      throw ApiError.badRequest('passwords are not correct');
