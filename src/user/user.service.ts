...
export class UserService {
  ...
  async createModerator(moderatorDto: ModeratorDto): Promise<UserEntity> {
    const candidate: UserEntity = await this.userRepository.findOne({
      where: { email: moderatorDto.email },
    });
    if (candidate) throw ApiError.badRequest('this user is already taken');
    moderatorDto.password = hashPasswordByBcrypt(moderatorDto.password);
