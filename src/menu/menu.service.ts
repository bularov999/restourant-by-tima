...
export class MenuService {
  ...
  async createMenu(menuDto: CreateMenuDto): Promise<MenuEntity> {
    menuDto.pictures = await this.fileService.findFilesById({
      where: { id: In(menuDto.pictureIds) },
    });
  ...
  async updateMenu(updateMenuDto: UpdateMenuDto): Promise<MenuEntity> {
    const oldMenu = await this.menuRepository.findOneBy({
      id: updateMenuDto.id,
    });
    updateMenuDto.pictures = await this.fileService.findFilesById({
      where: { id: In(updateMenuDto.picturesId) },
    });
